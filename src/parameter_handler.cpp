...
void ParameterHandler::displayHelp() {
    std::cerr << "Usage: openpose --input <file> --output <file> [OPTIONS]\n"
              << "Options:\n"
...
float ParameterHandler::parseFloat(const std::string& s, const std::string& arg_name) {
    try {
        float val = std::stof(s);
        return val;
    } catch (const std::invalid_argument&) {
        throw ParameterException("Invalid numeric value for " + arg_name + ": " + s);
    } catch (const std::out_of_range&) {
        throw ParameterException("Value for " + arg_name + " is out of range: " + s);
    }
}
...
ProgramOptions ParameterHandler::parse(int argc, char** argv) {
    ...
    while ((opt = getopt_long(argc, argv, "i:o:b:t:m:M:vh", long_options, &option_index)) != -1) {
        switch (opt) {
        case 'i':
        ...
        case 'M': {
            ...
            while (getline(ss, mode, ',')) {
                ...
                if (mode == "body") opts.process_body = true;
                else if (mode == "hand" || mode == "hands") opts.process_hand = true;
                else if (mode == "face") opts.process_face = true;
                else if (mode == "all") {
    ...
    // If --mode was not used, default to body. If it was used but no valid modes were found, also default to body.
    if (!mode_was_set || (!opts.process_body && !opts.process_hand && !opts.process_face)) {
        opts.process_body = true;
    }
